...
def tensor_pool_adjusted_model(model, tensor_pool_fn):
  ...
  """
    if isinstance(model, namedtuples.GANModel):
        ...
        raise ValueError('Tensor pool does not support `model`: %s.' %
                         type(model))
...
def gan_loss(
  ...
  """
    ...
    dis_loss = discriminator_loss_fn(
        pooled_model, **_optional_kwargs(discriminator_loss_fn,
                                         possible_kwargs))
  ...
  """
    ...
    """
        ...
        with tf.compat.v1.name_scope(scope_name,
                                     values=(true_labels, predict_logits)):

            loss = classification_loss_fn(onehot_labels=true_labels,
...
def gan_train_ops(
  ...
  """
    ...
    if isinstance(discriminator_optimizer,
        ...
        sync_hooks.append(
            discriminator_optimizer.make_session_run_hook(is_chief))
...
def get_sequential_train_hooks(train_steps=namedtuples.GANTrainSteps(1, 1)):
  ...
  """
    def get_hooks(train_ops):
        ...
        return [generator_hook, discriminator_hook] + list(
  ...
  """
    ...
    """
        ...
        for _ in range(train_steps.generator_train_steps):
            cur_gen_loss, _ = train_step(sess, train_ops.generator_train_op,
...
def _convert_tensor_or_l_or_d(tensor_or_l_or_d):
    ...
    if isinstance(tensor_or_l_or_d, (list, tuple)):
        ...
        return {
            k: tf.convert_to_tensor(value=v)
            for k, v in tensor_or_l_or_d.items()
        }
    else:
    ...
    if 'should_log' in train_step_kwargs:
        if sess.run(train_step_kwargs['should_log']):
            tf.compat.v1.logging.info(
                'global step %d: loss = %.4f (%.3f sec/step)', np_global_step,
                total_loss, time_elapsed)
