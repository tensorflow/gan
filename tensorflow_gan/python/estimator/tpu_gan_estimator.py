...
class TPUGANEstimator(tf.contrib.tpu.TPUEstimator):
  ...
  def __init__(self,
    ...
    def _model_fn(features, labels, mode, params):
      ...
      gan_model = estimator_get_gan_model(
          mode,
          generator_fn,
          discriminator_fn,
          real_data,
...
def get_estimator_spec(mode, gan_model, generator_loss_fn,
...
def _combine_train_ops(gen_train_op, dis_train_op, joint_train,
  ...
  del gan_train_steps
  if joint_train:
    tpu_train_op = tf.group(gen_train_op(), dis_train_op(), name='joint_train')
  else:
