  ...
  """
    ...
    """
        ...
        def _model_fn(features, labels, mode, params):
            ...
            assert isinstance(
                estimator_spec, tf.compat.v1.estimator.tpu.TPUEstimatorSpec
            )
...
def _get_gan_model_fns(
    ...
    if mode == tf.estimator.ModeKeys.PREDICT:
        if real_data is not None:
            raise ValueError(
                "`labels` must be `None` when mode is `predict`. "
        ...
        gan_models = _make_gan_model_fns(
            generator_fn,
            discriminator_fn,
            real_data,
            generator_inputs,
...
def get_eval_estimator_spec(
    ...
    assert (
        len(gan_model_fns) == 1
    ), "`gan_models` must be length 1 in eval mode. Got length %d" % len(gan_model_fns)
    ...
    if not all(isinstance(t, tf.Tensor) for t in flat_tensors):
        raise ValueError(
            "All objects nested within the TF-GAN model must be "
...
def _get_train_op(
    ...
    for i in range(total_steps):
        ...
        with tf.control_dependencies([prev_op]):
            ...
            gan_loss = _get_loss_for_train(
                gan_model, loss_fns, gan_loss_kwargs, add_summaries
            )
...
def _maybe_make_cross_shard_optimizers(optimizers):
    ...
    return Optimizers(
        _maybe_make_cross_shard_optimizer(optimizers.gopt),
