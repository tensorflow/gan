...
class StarGetGANModelTest(tf.test.TestCase, parameterized.TestCase):
    ...
    def test_get_gan_model(self, mode):
        ...
        if mode == tf.estimator.ModeKeys.PREDICT:
            ...
            self.assertEqual(
                input_data_domain_label, gan_model.generated_data_domain_target
            )
...
def get_metrics(gan_model):
    return {
        "mse_custom_metric": tf.compat.v1.metrics.mean_squared_error(
            gan_model.input_data, gan_model.generated_data
        )
    }
...
class StarGANEstimatorIntegrationTest(tf.test.TestCase):
    ...
    def _test_complete_flow(
        ...
        self.assertEqual(
            scores["discriminator_loss"] + scores["generator_loss"], scores["loss"]
        )
    ...
    def test_numpy_input_fn(self):
        ...
        train_input_fn = tf.compat.v1.estimator.inputs.numpy_input_fn(
            x={"x": image_data}, batch_size=batch_size, num_epochs=None, shuffle=True
        )
        ...
        train_input_fn = self._numpy_input_fn_wrapper(
            train_input_fn, batch_size, label_size
        )
        eval_input_fn = self._numpy_input_fn_wrapper(
            eval_input_fn, batch_size, label_size
        )
