  ...
  """
    ...
    def compute_gradients(self, loss, var_list, *args, **kwargs):
        ...
        if self._first_call:
            ...
            assert_op = tf.Assert(
                ...
                [
                    self._name,
                    "encountered repeated loss at substep",
                    substep_counter,
                    "current loss:",
                    loss,
                    "previous loss:",
    ...
    def apply_gradients(self, grads_and_vars, global_step=None, name=None):
        ...
        self._opt._create_non_slot_variable(  # pylint:disable=protected-access
            initial_value=0, name="substep_counter", colocate_with=colocate_with
        )
        # Not shared
        self._create_non_slot_variable(
            initial_value=0, name="substep_mask", colocate_with=colocate_with
        )
...
def get_dummy_gan_model(generated_data=None):
    ...
    with tf.compat.v1.variable_scope(
        "generator", reuse=tf.compat.v1.AUTO_REUSE
    ) as gen_scope:
...
def prepare_arguments_for_metric_fn(
    generator_inputs,
    generated_data,
    real_data,
...
class GetTPUEstimatorSpecTest(tf.test.TestCase, parameterized.TestCase):
    ...
    def setUpClass(cls):
        ...
        cls._optimizers = Optimizers(
            cls._generator_optimizer, cls._discriminator_optimizer
        )

        cls._loss_fns = LossFns(
            tfgan.losses.wasserstein_generator_loss,
            tfgan.losses.wasserstein_discriminator_loss,
        )

    @parameterized.named_parameters(("joint_train", True), ("train_sequential", False))
...
class TPUGANEstimatorIntegrationTest(tf.test.TestCase, parameterized.TestCase):
    ...
    def test_numpy_input_fn(self, joint_train, lr_decay, return_ds):
        ...
        def train_input_fn(params):
            ...
            ds = (
                tf.data.Dataset.from_tensors((data, data))
                .repeat()
