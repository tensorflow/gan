  ...
  """
    ...
    """
        _validate_input_args(
            generator_loss_fn, discriminator_loss_fn, use_loss_summaries, get_hooks_fn
        )
...
def get_gan_model(
    ...
    if mode == tf.estimator.ModeKeys.PREDICT:
        ...
        gan_model = make_prediction_gan_model(
...
def get_eval_estimator_spec(gan_model, gan_loss, get_eval_metric_ops_fn=None):
    ...
    with tf.compat.v1.name_scope(name="metrics"):
        eval_metric_ops = {
            ...
            "discriminator_loss": tf.compat.v1.metrics.mean(
...
def get_predict_estimator_spec(gan_model):
    ...
    return tf.estimator.EstimatorSpec(
        mode=tf.estimator.ModeKeys.PREDICT, predictions=gan_model.generated_data
    )
...
def get_train_estimator_spec(
    ...
    train_ops = train_op_fn(
        gan_model, gan_loss, optimizers.gopt, optimizers.dopt, is_chief=is_chief
    )
