...
__all__ = ["StarGANEstimator", "SummaryType", "stargan_prediction_input_fn_wrapper"]
...
def get_gan_model(
    ...
    if mode == tf.estimator.ModeKeys.PREDICT:
        gan_model = _make_prediction_gan_model(
            input_data, input_data_domain_label, generator_fn, generator_scope
        )
...
def _make_prediction_gan_model(
    input_data, input_data_domain_label, generator_fn, generator_scope
):
...
def _get_eval_estimator_spec(
    gan_model, gan_loss, get_eval_metric_ops_fn=None, name=None
):
    ...
    with tf.compat.v1.name_scope(name="metrics"):
        ...
        eval_metric_ops = {
            _summary_key(name, "generator_loss"): tf.compat.v1.metrics.mean(
                gan_loss.generator_loss
            ),
            _summary_key(name, "discriminator_loss"): tf.compat.v1.metrics.mean(
