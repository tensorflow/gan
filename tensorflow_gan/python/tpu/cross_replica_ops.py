...
def cross_replica_moments(inputs, axis, parallel=True, group_size=None):
  ...
  """
    ...
    if parallel:
        ...
        # overhead.
        mean_of_squares = tf.reduce_mean(input_tensor=tf.square(inputs),
                                         axis=axis)
        mean_of_squares = cross_replica_mean(mean_of_squares,
                                             group_size=group_size)
        mean_squared = tf.square(mean)
        variance = mean_of_squares - mean_squared
    else:
        variance = tf.math.reduce_mean(input_tensor=tf.math.square(inputs -
                                                                   mean),
                                       axis=axis)
