...
def infogan_discriminator_model(inputs, _):
    return (
        tf.compat.v1.get_variable("dummy_d", initializer=2.0) * inputs,
...
def create_callable_cyclegan_model():
    return tfgan.cyclegan_model(
        Generator(), Discriminator(), data_x=tf.zeros([1, 2]), data_y=tf.ones([1, 2])
    )
...
class StarGANModelTest(tf.test.TestCase):
    ...
    def test_stargan_model_generator_output(self):
        ...
        with self.cached_session() as sess:
            ...
            self.assertTupleEqual(
                (batch_size * num_domains, img_size, img_size, c_size), input_data.shape
            )
            self.assertTupleEqual(
    ...
    def test_stargan_model_discriminator_output(self):
        ...
        with self.cached_session() as sess:
            ...
            self.assertEqual(
                batch_size * num_domains, disc_input_data_source_pred.shape[0]
            )
            self.assertEqual(1, len(disc_gen_data_source_pred.shape))
            self.assertEqual(
                batch_size * num_domains, disc_gen_data_source_pred.shape[0]
            )
            ...
            self.assertTupleEqual(
                (batch_size * num_domains, num_domains), input_label.shape
            )
            self.assertTupleEqual(
                (batch_size * num_domains, num_domains), disc_input_label.shape
            )
            self.assertTupleEqual(
                (batch_size * num_domains, num_domains), gen_label.shape
            )
            self.assertTupleEqual(
...
class GANLossTest(tf.test.TestCase, parameterized.TestCase):
    ...
    @parameterized.named_parameters(
        ("infogan", get_infogan_model, {"mutual_information_penalty_weight": 1.0}),
        (
            "acgan",
            get_acgan_model,
            {"aux_cond_generator_weight": 1.0, "aux_cond_discriminator_weight": 1.0},
        ),
    )
    def test_reduction(self, get_gan_model_fn, kwargs):
        if tf.executing_eagerly():
            ...
            return
        loss = tfgan.gan_loss(
            get_gan_model_fn(), reduction=tf.compat.v1.losses.Reduction.NONE, **kwargs
        )
        self.assertIsInstance(loss, tfgan.GANLoss)
    ...
    def test_grad_penalty(self, create_gan_model_fn, one_sided):
        ...
        loss_gp = tfgan.gan_loss(
            model, gradient_penalty_weight=1.0, gradient_penalty_one_sided=one_sided
        )
    ...
    @parameterized.named_parameters(
        ("infogan", get_infogan_model), ("callable_infogan", get_callable_infogan_model)
    )
    def test_mutual_info_penalty(self, create_gan_model_fn):
        """Test mutual information penalty option."""
        tfgan.gan_loss(
            create_gan_model_fn(), mutual_information_penalty_weight=tf.constant(1.0)
        )
    ...
    @parameterized.named_parameters(
        ("notcallable", create_acgan_model), ("callable", create_callable_acgan_model)
    )
    ...
    def test_cyclegan(self, create_gan_model_fn):
        ...
        with self.cached_session() as sess:
            ...
            (
                ...
                [
                    loss.loss_x2y.generator_loss,
                    loss.loss_x2y.discriminator_loss,
                    loss.loss_y2x.generator_loss,
...
class TensorPoolAdjusteModelTest(tf.test.TestCase):
    ...
    def _make_new_model_and_check(self, model, pool_size):
        ...
        self.assertIsNot(
            new_model.discriminator_gen_outputs, model.discriminator_gen_outputs
        )
    ...
    def test_tensor_pool_adjusted_model_infogan(self):
        ...
        # Check values.
        self.assertIsNot(
            new_model.predicted_distributions, model.predicted_distributions
        )
        self._check_tensor_pool_adjusted_model_outputs(
            model.discriminator_gen_outputs,
            new_model.discriminator_gen_outputs,
    ...
    def test_tensor_pool_adjusted_model_acgan(self):
        ...
        # Check values.
        self.assertIsNot(
            new_model.discriminator_gen_classification_logits,
...
class GANTrainOpsTest(tf.test.TestCase, parameterized.TestCase):
    ...
    def test_unused_update_ops(self, create_gan_model_fn, provide_update_ops):
        ...
        with tf.compat.v1.variable_scope(model.generator_scope):
            ...
            tf.compat.v1.add_to_collection(
                tf.compat.v1.GraphKeys.UPDATE_OPS, gen_update_op
            )
        with tf.compat.v1.variable_scope(model.discriminator_scope):
            ...
            tf.compat.v1.add_to_collection(
                tf.compat.v1.GraphKeys.UPDATE_OPS, dis_update_op
            )
        ...
        if provide_update_ops:
            ...
            tf.compat.v1.add_to_collection(
                tf.compat.v1.GraphKeys.UPDATE_OPS, tf.constant(1.0)
            )
            kwargs = {}
    ...
    @parameterized.named_parameters(("is_chief", True), ("is_not_chief", False))
