...
def laplacian_pyramid(batch, num_levels):
  ...
  """
    ...
    def pyr_up(batch):  # matches cv2.pyrUp()
        ...
        res = contrib.batch_to_space(input=res,
                                     crops=[[0, 0], [0, 0]],
                                     block_shape=2)
...
def _batch_to_patches(batch, patches_per_image, patch_size):
  ...
  """
    ...
    def py_func_random_patches(batch):
        ...
        patch_id, y, x, chan = np.ogrid[0:patch_count, -hs:hs + 1, -hs:hs + 1,
                                        0:3]
    ...
    patches = tf.compat.v1.py_func(py_func_random_patches, [batch],
                                   batch.dtype,
                                   stateful=False)
    return patches
...
def sliced_wasserstein_distance(real_images,
  ...
  """
    ...
    # Base loss of detail.
    resolutions = [
        2**i for i in range(int(np.log2(resolution_max)),
