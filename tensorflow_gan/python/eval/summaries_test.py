...
class SummariesTest(tf.test.TestCase):
    ...
    def _test_add_gan_model_summaries_impl(
        self, get_model_fn, expected_num_summary_ops
    ):
        if tf.executing_eagerly():
            return
    ...
    def _test_add_regularization_loss_summaries_impl(
        self, get_model_fn, expected_num_summary_ops
    ):
        if tf.executing_eagerly():
            return
    ...
    def _test_add_image_comparison_summaries_impl(
        self, get_model_fn, expected_num_summary_ops
    ):
        if tf.executing_eagerly():
            return
