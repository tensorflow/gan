...
def python_image_grid(input_array, grid_shape, image_shape=(32, 32),
                      num_channels=3):
  """This is a pure python version of tfgan.eval.image_grid.

  Args:
    input_array: ndarray. Minibatch of images to format, either 4D
        ([batch size, height, width, num_channels]) or flattened
        ([batch size, height * width * num_channels]).
    grid_shape: Sequence of int. The shape of the image grid,
        formatted as [grid_height, grid_width].
  ...
  Returns:
    ndarray representing a single image in which the input images have been
    arranged into a grid.
  ...
  if len(input_array.shape) == 2:
    num_features = image_shape[0] * image_shape[1] * num_channels
    if int(input_array.shape[1]) != num_features:
      raise ValueError("Image shape and number of channels incompatible with "
                       "input tensor.")
  elif len(input_array.shape) == 4:
    if (int(input_array.shape[1]) != image_shape[0] or
        int(input_array.shape[2]) != image_shape[1] or
        int(input_array.shape[3]) != num_channels):
      raise ValueError("Image shape and number of channels incompatible with "
                       "input tensor.")
  else:
    raise ValueError("Unrecognized input tensor format.")
