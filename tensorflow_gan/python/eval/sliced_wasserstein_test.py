...
class ClassifierMetricsTest(tf.test.TestCase):
    def test_laplacian_pyramid(self):
        ...
        def np_pyr_down(minibatch):  # matches cv2.pyrDown()
            ...
            return ndimage.convolve(minibatch,
                                    gaussian_filter[np.newaxis,
                                                    np.newaxis, :, :],
                                    mode='mirror')[:, :, ::2, ::2]
        ...
        for x in range(3):
            self.assertAllClose(pyramid[x].transpose(0, 2, 3, 1),
                                pyramid_tf[x],
                                atol=1e-6)
    ...
    def test_sliced_wasserstein_distance(self):
        ...
        self.assertAllClose(np.array([0.014, 0.014], 'f'),
                            np.array([x[0] for x in wscores], 'f'),
                            rtol=0.15)
        self.assertAllClose(np.array([0.014, 0.020], 'f'),
    ...
    def test_sliced_wasserstein_distance_svd(self):
        ...
        with self.cached_session() as sess:
            wscores = [sess.run(x) for x in wfunc]
        self.assertAllClose(np.array([0.013, 0.013], 'f'),
                            np.array([x[0] for x in wscores], 'f'),
                            rtol=0.15)
        self.assertAllClose(np.array([0.014, 0.019], 'f'),
                            np.array([x[1] for x in wscores], 'f'),
                            rtol=0.15)
