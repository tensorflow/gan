...
__all__ = [
    'add_gan_model_image_summaries', 'add_image_comparison_summaries',
    'add_gan_model_summaries', 'add_regularization_loss_summaries',
    'add_cyclegan_image_summaries', 'add_stargan_image_summaries'
]
...
def add_cyclegan_image_summaries(cyclegan_model):
  ...
  """
    if not isinstance(cyclegan_model, namedtuples.CycleGANModel):
        raise ValueError(
            '`cyclegan_model` was not a CycleGANModel. Instead, was '
    ...
    def _add_comparison_summary(gan_model, reconstructions):
        ...
        tf.compat.v1.summary.image('image_comparison',
                                   eval_utils.image_reshaper(
                                       image_list, num_cols=len(image_list)),
                                   max_outputs=1)

    with tf.compat.v1.name_scope('x2y_image_comparison_summaries'):
        _add_comparison_summary(cyclegan_model.model_x2y,
                                cyclegan_model.reconstructed_x)
    with tf.compat.v1.name_scope('y2x_image_comparison_summaries'):
        _add_comparison_summary(cyclegan_model.model_y2x,
                                cyclegan_model.reconstructed_y)
...
def add_image_comparison_summaries(gan_model,
  ...
  """
    ...
    if display_diffs:
        ...
        diffs = [
            tf.abs(_to_float(generated) - _to_float(real))
    ...
    tf.compat.v1.summary.image('image_comparison',
                               eval_utils.image_reshaper(
                                   image_list, num_cols=num_comparisons),
                               max_outputs=1)
...
def add_stargan_image_summaries(stargan_model,
  ...
  """
    ...
    tf.compat.v1.summary.image('stargan_image_generation',
  ...
  """
    if isinstance(gan_model, namedtuples.CycleGANModel):
        with tf.compat.v1.name_scope(
                'cyclegan_x2y_regularization_loss_summaries'):
            add_regularization_loss_summaries(gan_model.model_x2y)
        with tf.compat.v1.name_scope(
                'cyclegan_y2x_regularization_loss_summaries'):
