...
class SpectralNormalizationTest(tf.test.TestCase, parameterized.TestCase):
    ...
    def testSpectralNormalize(self):
        ...
        normalized_sigma = tf.linalg.svd(
            tf.reshape(normalized_weights, [-1, weights.shape[-1]]), compute_uv=False
        )[..., 0]
        ...
        with self.cached_session() as sess:
            ...
            self.assertAlmostEqual(1.0, s50, 0)
            self.assertGreater(abs(s10 - 1.0), abs(s50 - 1.0))
            self.assertGreater(abs(s5 - 1.0), abs(s10 - 1.0))
            self.assertGreater(abs(s1 - 1.0), abs(s5 - 1.0))
            self.assertGreater(abs(s0 - 1.0), abs(s1 - 1.0))
    ...
    def testSpectralNormalizeZeroMatrix(self):
        ...
        self.assertAllClose(normalized_w_norm, 0.0)
    ...
    def testSpectralNormalizeTinyMatrix(self):
        ...
        with self.cached_session() as sess:
            ...
            np_normalized_w = sess.run(normalized_w)
        normalized_w_norm = np.linalg.svd(np_normalized_w.reshape([-1, 3]))[1][0]
        self.assertAllClose(normalized_w_norm, 1.0, rtol=1e-4, atol=1e-4)
    ...
    def _testLayerHelper(self, build_layer_fn, w_shape, b_shape):
        ...
        with context_manager:
            ...
            ) = build_layer_fn(x, w_initializer, b_initializer)
        ...
        with self.cached_session() as sess:
            ...
            for name, var in expected_not_normalized_vars.items():
                ...
                self.assertTrue(
                    ...
                    msg=(
                        "%s appears to be unexpectedly normalized. "
            ...
            for w_before, (name, var) in zip(
                w_befores, expected_normalized_vars.items()
            ):
                w_after = sess.run(var)
                self.assertFalse(
                    np.allclose(w_before, w_after, rtol=1e-8, atol=1e-8),
                    msg=(
                        "%s did not improve over many iterations. "
                ...
                self.assertAllClose(
                    ...
                    atol=1e-4,
                    msg=(
                        "Estimate of spectral norm for %s was innacurate. "
