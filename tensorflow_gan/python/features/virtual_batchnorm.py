...
def vbn_statistics(x, axes):
  ...
  """
    ...
    shifted_mean = tf.reduce_mean(input_tensor=y - shift,
                                  axis=axes,
                                  keepdims=True)
    mean = shifted_mean + shift
    mean_squared = tf.reduce_mean(input_tensor=tf.square(y),
                                  axis=axes,
                                  keepdims=True)
...
class VBN(object):
  ...
  """
    ...
    """
        ...
        with tf.compat.v1.variable_scope(name, 'VBN',
            ...
            self._ref_variance = self._ref_mean_squares - tf.square(
                self._ref_mean)
            ...
            self._example_weight = 1. / (tf.cast(ref_batch_size, tf.float32) +
                                         1.)
    ...
    def reference_batch_normalization(self):
        ...
        with tf.compat.v1.name_scope(self._vs.name):
            return tf.nn.batch_normalization(
                self._reference_batch, self._broadcast(self._ref_mean),
