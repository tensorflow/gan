...
__all__ = ["group_norm", "instance_norm"]
...
def group_norm(
  ...
  """
    ...
    if channels is None:
        raise ValueError(
            "Inputs %s has undefined channel dimension: %d."
    ...
    for a in reduction_axes:
        ...
        if channels_axis == a:
            raise ValueError(
                "reduction_axis must be mutually exclusive " "with channels_axis"
            )
    if groups > channels:
