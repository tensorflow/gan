...
def wasserstein_generator_loss(
  ...
  """
    with tf.compat.v1.name_scope(
        scope, "generator_wasserstein_loss", (discriminator_gen_outputs, weights)
    ) as scope:
...
def wasserstein_discriminator_loss(
  ...
  """
    with tf.compat.v1.name_scope(
        scope,
        "discriminator_wasserstein_loss",
        (
            discriminator_real_outputs,
            discriminator_gen_outputs,
            real_weights,
        ...
        if add_summaries:
            tf.compat.v1.summary.scalar(
                "discriminator_gen_wass_loss", loss_on_generated
            )
...
def wasserstein_hinge_discriminator_loss(
  ...
  """
    with tf.compat.v1.name_scope(
        scope,
        "discriminator_wasserstein_hinge_loss",
        (
            discriminator_real_outputs,
            discriminator_gen_outputs,
            real_weights,
        ...
        # Average.
        loss_on_real = tf.compat.v1.losses.compute_weighted_loss(
            hinged_real, real_weights, scope, loss_collection=None, reduction=reduction
        )
        loss_on_generated = tf.compat.v1.losses.compute_weighted_loss(
            hinged_gen,
            generated_weights,
        ...
        if add_summaries:
            tf.compat.v1.summary.scalar(
                "discriminator_gen_wass_hinge_loss", loss_on_generated
            )
            tf.compat.v1.summary.scalar(
                "discriminator_real_wass_hinge_loss", loss_on_real
            )
...
def acgan_generator_loss(
  ...
  """
    with tf.compat.v1.name_scope(
        scope,
        "acgan_generator_loss",
        (discriminator_gen_classification_logits, one_hot_labels),
    ) as scope:
...
def wasserstein_gradient_penalty(
  ...
  """
    ...
    with tf.compat.v1.name_scope(
        scope, "wasserstein_gradient_penalty", (real_data, generated_data)
    ) as scope:
        ...
        batch_size = (
            tf.compat.dimension_value(differences.shape.dims[0])
        ...
        with tf.compat.v1.name_scope(
            ...
            with tf.compat.v1.variable_scope(
                discriminator_scope, "gpenalty_dscope", reuse=tf.compat.v1.AUTO_REUSE
            ):
        ...
        if one_sided:
            penalties = tf.maximum(0.0, penalties)
...
def modified_generator_loss(
  ...
  """
    with tf.compat.v1.name_scope(
        scope, "generator_modified_loss", [discriminator_gen_outputs]
    ) as scope:
...
def least_squares_generator_loss(
  ...
  """
    with tf.compat.v1.name_scope(
        scope, "lsq_generator_loss", (discriminator_gen_outputs, real_label)
    ) as scope:
        discriminator_gen_outputs = _to_float(discriminator_gen_outputs)
...
def least_squares_discriminator_loss(
  ...
  """
    with tf.compat.v1.name_scope(
        scope, "lsq_discriminator_loss", (discriminator_gen_outputs, real_label)
    ) as scope:
        discriminator_real_outputs = _to_float(discriminator_real_outputs)
        discriminator_gen_outputs = _to_float(discriminator_gen_outputs)
        discriminator_real_outputs.shape.assert_is_compatible_with(
        ...
        real_losses = (
            tf.math.squared_difference(discriminator_real_outputs, real_label) / 2.0
        )
        fake_losses = (
            tf.math.squared_difference(discriminator_gen_outputs, fake_label) / 2.0
        )
...
def _validate_distributions(distributions):
    ...
    if not isinstance(distributions, (list, tuple)):
        raise ValueError(
            "`distributions` must be a list or tuple. Instead, "
...
def numerically_stable_global_norm(tensor_list):
  ...
  """
    ...
    list_max = tf.reduce_max(
        input_tensor=[
            tf.reduce_max(input_tensor=tf.abs(x)) for x in tensor_list if x is not None
        ]
    )
