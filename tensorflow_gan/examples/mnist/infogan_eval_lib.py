...
def evaluate(hparams, run_eval_loop=True):
  ...
  """
    ...
    with tf.compat.v1.variable_scope(
        "Generator"
    ) as genscope:  # Same scope as in training.
    ...
    tf.compat.v1.summary.image("categorical", reshaped_categorical_img, max_outputs=1)
    ...
    tf.compat.v1.summary.image("continuous1", reshaped_continuous1_img, max_outputs=1)
    ...
    tf.compat.v1.summary.image("continuous2", reshaped_continuous2_img, max_outputs=1)

    # Evaluate image quality.
    all_images = tf.concat(
        [categorical_images, continuous1_images, continuous2_images], 0
    )
    tf.compat.v1.summary.scalar("MNIST_Classifier_score", util.mnist_score(all_images))
    ...
    if hparams.write_to_disk:
        image_write_ops.append(
            _get_write_image_ops(
                hparams.eval_dir, "categorical_infogan.png", reshaped_categorical_img[0]
            )
        )
        image_write_ops.append(
            _get_write_image_ops(
                hparams.eval_dir, "continuous1_infogan.png", reshaped_continuous1_img[0]
            )
        )
        image_write_ops.append(
            _get_write_image_ops(
                hparams.eval_dir, "continuous2_infogan.png", reshaped_continuous2_img[0]
            )
        )
