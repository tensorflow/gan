...
class LayersTest(tf.test.TestCase):
    def setUp(self):
        ...
        # mocking it in tests.
        _ = tf.compat.v1.layers.conv2d(np.ones(shape=[1, 3, 3, 1],
                                               dtype=np.float32),
                                       filters=1,
                                       kernel_size=2)
    ...
    def test_he_initializer_scale_slope_linear(self):
        self.assertAlmostEqual(layers.he_initializer_scale([3, 4, 5, 6], 1.0),
                               0.1290994, 5)

    def test_he_initializer_scale_slope_relu(self):
        self.assertAlmostEqual(layers.he_initializer_scale([3, 4, 5, 6], 0.0),
                               0.1825742, 5)

    @mock.patch.object(tf.compat.v1,
    ...
    def test_custom_layer_impl_with_weight_scaling(
        ...
        mock_zeros_initializer.return_value = tf.compat.v1.constant_initializer(
            1.0)
        mock_random_normal_initializer.return_value = (
            tf.compat.v1.constant_initializer(3.0))
        output = layers._custom_layer_impl(apply_kernel=dummy_apply_kernel,
                                           kernel_shape=(25, 6),
                                           bias_shape=(),
    ...
    def test_custom_conv2d_passes_conv2d_options(self, mock_conv2d):
        ...
        mock_conv2d.assert_called_once_with(x,
                                            filters=1,
                                            kernel_size=[2, 2],
                                            strides=(1, 1),
                                            padding='SAME',
    ...
    def test_custom_conv2d_scalar_kernel_size(self, mock_zeros_initializer,
        ...
        expected_np = [[[[68.54998016], [42.56921768]],
                       ...
                       [[[5.33012676], [4.46410179]],
                        [[10.52627945], [9.66025352]]]]
        self.assertAllClose(output_np, expected_np, 1.0e-5)
