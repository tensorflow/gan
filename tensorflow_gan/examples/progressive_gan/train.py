...
def get_stage_ids(**kwargs):
  ...
  """
    train_sub_dirs = [
        sub_dir
        for sub_dir in tf.io.gfile.listdir(kwargs["train_log_dir"])
...
def add_generator_smoothing_ops(generator_ema, gan_model, gan_train_ops):
    ...
    gan_train_ops = gan_train_ops._replace(generator_train_op=new_generator_train_op)
...
def build_model(stage_id, batch_size, real_images, **kwargs):
  ...
  """
    ...
    def _num_filters_fn(block_id):
        ...
        return networks.num_filters(
            block_id, kwargs["fmap_base"], kwargs["fmap_decay"], kwargs["fmap_max"]
        )
    ...
    ########## Define train ops.
    gan_train_ops, optimizer_var_list = define_train_ops(gan_model, gan_loss, **kwargs)
    gan_train_ops = gan_train_ops._replace(global_step_inc_op=current_image_id_inc_op)
...
def add_model_summaries(model, **kwargs):
  ...
  """
    ...
    with tf.compat.v1.variable_scope(
        ...
        z_interp = make_interpolated_latent_vectors(
            interp_grid_size, interp_grid_size, **kwargs
        )
...
def make_status_message(model):
    ...
    return tf.strings.join(
        [
            ...
            ", num_blocks: {}".format(model.num_blocks),
