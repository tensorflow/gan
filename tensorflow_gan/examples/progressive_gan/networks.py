...
class ResolutionSchedule(object):
    ...
    def scale_factor(self, block_id):
        ...
        if block_id < 1 or block_id > self._num_resolutions:
            raise ValueError(
                "`block_id` must be in [1, {}]".format(self._num_resolutions)
            )
...
def min_total_num_images(
    stable_stage_num_images, transition_stage_num_images, num_blocks
):
  ...
  """
    return (
        num_blocks * stable_stage_num_images
        + (num_blocks - 1) * transition_stage_num_images
    )


def compute_progress(
    current_image_id, stable_stage_num_images, transition_stage_num_images, num_blocks
):
...
def _generator_alpha(block_id, progress):
  ...
  """
    return tf.maximum(0.0, tf.minimum(progress - (block_id - 2), block_id - progress))
    ...
    with tf.compat.v1.variable_scope(scope, reuse=reuse):
        ...
        with tf.compat.v1.variable_scope(block_name(1)):
            ...
            x = _conv2d(
                "conv1",
                x,
                resolution_schedule.start_resolutions,
                num_filters_fn(0),
                "VALID",
            )
