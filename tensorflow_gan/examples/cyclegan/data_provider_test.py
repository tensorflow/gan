...
class DataProviderTest(tf.test.TestCase):
    def setUp(self):
        ...
        self.testdata_dir = os.path.join(
            flags.FLAGS.test_srcdir, "tensorflow_gan/examples/cyclegan/testdata"
        )
    ...
    def test_custom_datasets_provider(self):
        ...
        for images_ds in images_ds_list:
            self.assertListEqual(
                [None, patch_size, patch_size, 3], images_ds.output_shapes.as_list()
            )
        ...
        with self.cached_session() as sess:
            ...
            for images_out in images_out_list:
                self.assertTupleEqual(
                    (batch_size, patch_size, patch_size, 3), images_out.shape
                )
    ...
    def test_custom_data_provider(self):
        ...
        for images in images_list:
            self.assertListEqual(
                [batch_size, patch_size, patch_size, 3], images.shape.as_list()
            )
