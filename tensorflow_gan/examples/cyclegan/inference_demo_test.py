...
def _basenames_from_glob(file_glob):
    return [os.path.basename(file_path) for file_path in tf.io.gfile.glob(file_glob)]
...
class InferenceDemoTest(tf.test.TestCase):
    def setUp(self):
        ...
        self._image_glob = os.path.join(
            FLAGS.test_srcdir, "tensorflow_gan/examples/cyclegan/testdata", "*.jpg"
        )
    ...
    @mock.patch.object(train_lib.data_provider, "provide_custom_data", autospec=True)
