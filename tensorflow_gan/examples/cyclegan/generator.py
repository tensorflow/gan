...
def _instance_norm(x):
    return tfgan.features.instance_norm(x, center=True, scale=True, epsilon=0.001)


def cyclegan_upsample(
    net, num_outputs, strides, method="conv2d_transpose", pad_mode="REFLECT"
):
    ...
    with tf.compat.v1.variable_scope("upconv"):
        ...
        if method == "nn_upsample_conv":
            ...
            net = tf.compat.v1.layers.conv2d_transpose(
                net, num_outputs, kernel_size=[3, 3], strides=strides, padding="valid"
            )
            net = tf.nn.relu(net)
            net = net[:, 1:, 1:, :]
        else:
