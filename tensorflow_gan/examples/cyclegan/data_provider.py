...
def _provide_custom_dataset(image_file_pattern, num_threads=1):
  ...
  """
    ...
    images_ds = bytes_ds.map(tf.image.decode_image, num_parallel_calls=num_threads)
    return images_ds


def _preprocess_datasets(
    dataset, batch_size, shuffle=True, num_threads=1, patch_size=128
):
  ...
  """
    patches_ds = dataset.map(
        lambda img: full_image_to_patch(img, patch_size), num_parallel_calls=num_threads
    )
...
def provide_custom_datasets(
    batch_size, image_file_patterns=None, shuffle=True, num_threads=1, patch_size=128
):
  ...
  """
    if image_file_patterns and not isinstance(image_file_patterns, (list, tuple)):
        raise ValueError(
            "`image_file_patterns` should be either list or tuple, but was {}.".format(
        ...
        for pattern in image_file_patterns:
            images_ds.append(
                _provide_custom_dataset(
                    image_file_pattern=pattern, num_threads=num_threads
                )
            )
    else:
...
def provide_custom_data(
  ...
  """
    datasets = provide_custom_datasets(
        batch_size, image_file_patterns, shuffle, num_threads, patch_size
    )
