...
class MultiStepStopAfterNEvalsHook(tf.estimator.SessionRunHook):
    ...
    def before_run(self, run_context):
        return tf.estimator.SessionRunArgs({"evals_completed": self._evals_completed})
    ...
    def after_run(self, run_context, run_values):
        ...
        if self._num_evals is None:
            ...
            tf.compat.v1.logging.info(
                "Evaluation [%d/%d]", evals_completed, self._num_evals
            )
...
class SummaryAtEndHook(tf.estimator.SessionRunHook):
    ...
    def __init__(
        self, log_dir=None, summary_writer=None, summary_op=None, feed_dict=None
    ):
...
def evaluate_repeatedly(
  ...
  """
    ...
    for checkpoint_path in checkpoints_iterator(
        ...
        if (
            max_number_of_evaluations is not None
