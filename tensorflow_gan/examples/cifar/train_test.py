...
class TrainTest(tf.test.TestCase, parameterized.TestCase):
  ...
  @parameterized.parameters(
      {'conditional': False, 'use_sync_replicas': False},
      {'conditional': True, 'use_sync_replicas': False},
      {'conditional': False, 'use_sync_replicas': True},
  )
  @mock.patch.object(train, 'data_provider', autospec=True)
  def test_build_graph(self, mock_data_provider, conditional,
                       use_sync_replicas):
    ...
    mock_lbls = np.concatenate(
        (np.ones([FLAGS.batch_size, 1], dtype=np.float32),
         np.zeros([FLAGS.batch_size, 9], dtype=np.float32)), axis=1)
