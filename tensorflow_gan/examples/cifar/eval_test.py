...
class EvalTest(tf.test.TestCase, parameterized.TestCase):
    @parameterized.parameters({"eval_real_images": True}, {"eval_real_images": False})
    ...
    def test_build_graph(
        self, mock_provide_data, mock_iscore, mock_fid, eval_real_images
    ):
        ...
        mock_provide_data.return_value = (
            tf.ones([hparams.num_images_generated, 32, 32, 3]),
            tf.zeros([hparams.num_images_generated]),
        )
