...
def get_tpu_run_config_from_hparams(hparams):
    ...
    if hparams.debug_params.eval_on_tpu:
        eval_training_input_configuration = (
            tf.compat.v1.estimator.tpu.InputPipelineConfig.SLICED
        )
    else:
...
def get_gpu_estimator(generator, discriminator, hparams, config):
    ...
    def gpu_get_metric(gan_model):
        ...
        real_images = real_data["images"] if isinstance(real_data, dict) else real_data
        gen_images = (
...
def prepare_metric_arguments(
  ...
  """
    ...
    real_images = real_data["images"] if isinstance(real_data, dict) else real_data
    gen_images = (
        generated_data["images"] if isinstance(generated_data, dict) else generated_data
    )
...
def get_metrics(real_logits, real_pools, fake_logits, fake_pools, hparams):
  ...
  """
    del hparams
    metric_dict = {
        "eval/real_incscore": tfgan.eval.classifier_score_from_logits_streaming(
            real_logits
        ),
        "eval/incscore": tfgan.eval.classifier_score_from_logits_streaming(fake_logits),
...
def _generator_summary_ops(generated_images, real_images):
    ...
    real_img_grid = tf.compat.v1.summary.image(
        ...
        tfgan.eval.image_grid(
            real_images[:16], grid_shape=(4, 4), image_shape=(128, 128), num_channels=3
        ),
    )
