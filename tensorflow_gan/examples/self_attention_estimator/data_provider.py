...
flags.DEFINE_string(
    "imagenet_data_dir", None, "A directory for TFDS ImageNet. If `None`, use default."
)
...
def provide_dataset(batch_size, shuffle_buffer_size, split="train"):
  ...
  """
    ...
    dataset = _load_imagenet_dataset(
        split, flags.FLAGS.imagenet_data_dir, shuffle_files=shuffle
    )
    if shuffle:
    ...
    dataset = dataset.map(
        _preprocess_dataset_record_fn(IMG_SIZE),
...
def provide_data(batch_size, num_batches, shuffle_buffer_size, split="train"):
  ...
  Returns:
    ...
    labels.
  """
    dataset = provide_dataset(batch_size * num_batches, shuffle_buffer_size, split)
    iterator = tf.compat.v1.data.make_one_shot_iterator(dataset)
    images, labels = iterator.get_next()
    images = tf.reshape(images, shape=[num_batches, batch_size, IMG_SIZE, IMG_SIZE, 3])
    labels = tf.reshape(labels, shape=[num_batches, batch_size, 1])
    batches = list(
        zip(tf.unstack(images, num=num_batches), tf.unstack(labels, num=num_batches))
    )
    return batches
...
def _load_imagenet_dataset(split, data_dir=None, shuffle_files=False):
    return tfds.load(
        "imagenet2012", split=split, data_dir=data_dir, shuffle_files=shuffle_files
    )
...
def _preprocess_dataset_record_fn(image_size):
    ...
    def _process_record(record):
        ...
        image = compat_utils.crop_and_resize(
            [image],
            boxes=[
                [normalized_y_min, normalized_x_min, normalized_y_max, normalized_x_max]
            ],
            box_ind=[0],
