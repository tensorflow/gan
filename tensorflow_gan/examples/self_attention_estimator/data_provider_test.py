...
class DataProviderTest(tf.test.TestCase, parameterized.TestCase):
    ...
    def test_provide_data_shape(self, mock_ds):
        ...
        mock_ds.return_value = tf.data.Dataset.from_tensors(
            np.zeros([128, 128, 3])
        ).map(lambda x: {"image": x, "label": 1})
        batches = data_provider.provide_data(
            batch_size=batch_size, num_batches=num_batches, shuffle_buffer_size=10
        )
    ...
    def test_preprocess_dataset_record_centering(self):
        ...
        image_size = 7
        process_fn = data_provider._preprocess_dataset_record_fn(image_size=image_size)
        processed_record = process_fn(dummy_record)
        processed_record[0].shape.assert_is_compatible_with([image_size, image_size, 3])
        # Test that output is all-ones (ignore the boundary in the check).
        self.assertAllEqual(
            processed_record[0][1:-1, 1:-1, :],
    ...
    def test_compare_preprocess_with_improved_gan(self, nrows, ncols):
        ...
        dummy_record = {"image": tf.constant(test_image, dtype=tf.uint8), "label": [4]}
        process_fn = data_provider._preprocess_dataset_record_fn(image_size=128)
        processed_record = process_fn(dummy_record)
        with self.session():
            ...
            self.assertLess(
                tf.norm(tensor=improved_image - processed_record[0], ord=np.inf).eval(),
                4.0 / 256.0,
            )
