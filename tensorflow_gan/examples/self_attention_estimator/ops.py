...
def snconv2d(
    input_, output_dim, k_h=3, k_w=3, d_h=2, d_w=2, training=True, name="snconv2d"
):
  ...
  """
    with tf.compat.v1.variable_scope(
        name, custom_getter=sn_gettr(training=training, equality_constrained=False)
    ):
  ...
  """
    with tf.compat.v1.variable_scope(name):
        ...
        embedding_map_bar_transpose = tfgan.features.spectral_normalize(
            tf.transpose(a=embedding_map), training=training, equality_constrained=False
        )
...
def sn_conv1x1(x, output_dim, training=True, name="sn_conv1x1"):
  ...
  """
    with tf.compat.v1.variable_scope(name, custom_getter=sn_gettr(training=training)):
