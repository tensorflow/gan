...
def usample(x):
  ...
  """
    ...
    return tf.reshape(after_tile, [-1, image_height * 2, image_width * 2, n_channels])
...
def block(x, labels, out_channels, num_classes, name, training=True):
  ...
  """
    with tf.compat.v1.variable_scope(name):
        ...
        x_0 = x
        x = tf.nn.relu(tfgan.tpu.batch_norm(x, training, labels_onehot, name="cbn_0"))
        x = usample(x)
        x = ops.snconv2d(x, out_channels, 3, 3, 1, 1, training, "snconv1")
        x = tf.nn.relu(tfgan.tpu.batch_norm(x, training, labels_onehot, name="cbn_1"))
...
def generator(zs, target_class, gf_dim, num_classes, training=True):
  ...
  """
    with tf.compat.v1.variable_scope(
        ...
        act1 = block(
            act0, target_class, gf_dim * 16, num_classes, "g_block1", training
        )  # 8
        act2 = block(
            act1, target_class, gf_dim * 8, num_classes, "g_block2", training
        )  # 16
        act3 = block(
            act2, target_class, gf_dim * 4, num_classes, "g_block3", training
        )  # 32
        act3 = ops.sn_non_local_block_sim(act3, training, name="g_ops")  # 32
        act4 = block(
            act3, target_class, gf_dim * 2, num_classes, "g_block4", training
        )  # 64
        act5 = block(
            act4, target_class, gf_dim, num_classes, "g_block5", training
        )  # 128
        act5 = tf.nn.relu(
            tfgan.tpu.batch_norm(
                act5, training, conditional_class_labels=None, name="g_bn"
            )
        )
