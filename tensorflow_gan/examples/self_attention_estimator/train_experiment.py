...
def train_eval_input_fn(mode, params):
    ...
    images_ds = images_ds.map(
        lambda img, lbl: {"images": img, "labels": lbl}
    )  # map to dict.
...
def run_train(hparams):
  ...
  """
    ...
    tf.compat.v1.logging.info(
        "Training until %i steps..." % hparams.max_number_of_steps
    )
    estimator.train(train_eval_input_fn, max_steps=hparams.max_number_of_steps)
    tf.compat.v1.logging.info(
        "Finished training %i steps." % hparams.max_number_of_steps
    )
...
def run_continuous_eval(hparams):
    ...
    for ckpt_str in evaluation.checkpoints_iterator(hparams.model_dir, timeout=timeout):
        tf.compat.v1.logging.info("Evaluating checkpoint: %s" % ckpt_str)
        estimator.evaluate(
            train_eval_input_fn, steps=hparams.num_eval_steps, name="eval_continuous"
        )
    ...
    while cur_step < max_step:
        if hparams.tpu_params.use_tpu_estimator:
            ...
            eval_lib.predict_and_write_images(
                estimator, train_eval_input_fn, hparams.model_dir, "step_%i" % cur_step
            )
...
def _get_generator(hparams):
    ...
    def generator(noise, mode):
        ...
        if hparams.debug_params.fake_nets:
            gen_imgs = tf.zeros([batch_size, 128, 128, 3]) * tf.compat.v1.get_variable(
...
def _get_discriminator(hparams):
    ...
    def discriminator(images_and_lbls, unused_conditioning, mode):
        ...
        if hparams.debug_params.fake_nets:
            ...
            logits = (
                tf.zeros([tf.shape(input=images)[0], 20])
                * tf.compat.v1.get_variable("dummy_d", initializer=2.0)
                * tf.reduce_mean(input_tensor=images)
            )
            discriminator_vars = ()
        else:
