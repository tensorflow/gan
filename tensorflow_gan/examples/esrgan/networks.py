...
def _conv_block(x, filters, activation=True):
  ...
  h = tf.keras.layers.Conv2D(filters, 
                             kernel_size=[3, 3], 
...
def _conv_block_d(x, out_channel):
  ...
  x = tf.keras.layers.Conv2D(out_channel, 
                             kernel_size=3, 
                             strides=1, 
                             padding='same', 
                             use_bias=False)(x)
...
def generator_network(hparams,
  ...
  x = tf.keras.layers.Conv2D(num_filters, kernel_size=[3, 3], strides=[1, 1],
  ...
  x = tf.keras.layers.Conv2D(num_filters, 
                             ...
                             use_bias=True)(x)
  x = tf.keras.layers.LeakyReLU(0.2)(x)
  hr_output = tf.keras.layers.Conv2D(out_channels, 
                                     kernel_size=[3, 3], 
                                     strides=[1, 1],
                                     padding='same', 
                                     use_bias=True)(x)
...
def discriminator_network(hparams, num_filters=64):
  ...
  """
  img = tf.keras.layers.Input(shape=(hparams.hr_dimension, 
                                     hparams.hr_dimension, 3))
